...
- **The stdout contract.** From `record` and `stop`, stdout carries one line:
  ...
  to stdout too; what is protected there is that `--version` prints a parsable
  version and that both flags exist, not the prose either of them prints.
  Rewording help text stops nothing working, which is the governing standard's
  test (`versioning.md` § 2).
